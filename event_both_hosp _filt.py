...
if __name__ == "__main__":
    ...
    for i in range(len(df3['Job'])):
        #df3.at[i, 'In'] = time(df3['In'][i]) #for teneto measures computation
        df3.at[i, 'Job'] = node2index[df3['Job'][i]]
        df3.at[i, 'Receiver_Job'] = node2index[df3['Receiver_Job'][i]] #change to indexes
        
    edge_filterdf=df2.groupby(["Job", "Receiver_Job"]).size().reset_index(name="Count") # get filtered edges based on fully expanded dataframe
    ...
    for i in range(len(df3['Job'])):
        if df3['Receiver_Job'][i]!=-1 and df3['Job'][i]!=-1:
            if arr[df3['Job'][i]][df3['Receiver_Job'][i]]==1: #check with significance test case
    ...
    event=0
    event_lst=[]
    for i in range(len(mod_df['Time'])):
        event_lst.append(event)
        if i!=(len(mod_df['Job'])-1):
            if time(mod_df['Time'][i])<time(mod_df['Time'][i+1]):
                event+=1
    ...
    temp_wt=[] 
    thresh=1
    from_event, to_event, temp_wt=max_tempstep_filt(mod_df,thresh)
    print('tempstep')
    print(from_event)
    print(to_event)
    ...
    for i in range(len(mod_df['event'])):
        ...
        print(i)
        if mod_df['Job'][i]!=-1:
            pos[i]=(val,ii.index(mod_df['Job'][i])) #in increasing order of time
            pos_1[i]=(val,ii_1[mod_df['Job'][i]]) #in increasing order of time
        else:
    ...
    plt.savefig(argv[1][:-3] + '_eventlayoutrn1chrfilt.png', format = 'png', bbox_inches = 'tight')
