...
if __name__ == "__main__":
    ...
    df2['event']=event_lst
    for i in range(len(df2['From'])):
        if arr[df2['From'][i]][df2['To'][i]]==1: #check with significance test case
            mod_df=pd.concat([mod_df,df2.iloc[[i]]], ignore_index=True)
    
    print("Updated dataframe")
    print(mod_df)
    
    netin = {'i': mod_df['From'], 'j': mod_df['To'], 't': mod_df['In']}
    ...
    print(node_position)
    weighted_agg=mod_df.groupby(["From", "To"], as_index=False)["timed_duration"].sum() #sum total duration of interactions in seconds
    
    G = nx.MultiDiGraph()
    nodelist2=[i for i in range(len((mod_df['To']).unique()))]
    ...
    print("Modified Edge weights as values and edges as keys (dict)")
    ...
    try:
        #metric_df=format_metric(mod_df)
        #print("Temporal Betweenness centrality dict:")
        #betw_dict=get_between_dict(metric_df) #vals for node colour gradient
        #print(betw_dict)
        print("Temporal Closeness centrality dict:")
        close_dict=get_closeness_dict2(mod_df) #proportionate to node mass
        print(close_dict)
        ...
        for node, (x, y) in node_position.items():
            ...
            text(x, y+10, list(node2index.keys())[list(node2index.values()).index(node)], fontsize=val*2,ha='center', va='center')
    ...
    # title="Structural backbone area minimisation layout (pre-proc) "+ argv[1][:-4]
    # plt.title(title, fontsize=20)
    plt.savefig(argv[1][:-3] + '_structuralfilt4_2.pdf', format = 'pdf', bbox_inches = 'tight')
    plt.show()
