#both area minimisation and recurrent neighbours node reordering methods trialled and outputted for full temporal layouts of team datasets
...
def gen_plot(node_yticks,label_yticks,one2one,one2all): 
    ...
    ax.set_ylim(0, n)
    #ax.xaxis.set_major_locator(tck.MultipleLocator(base=10.0))
    #ax.set_xticklabels([])
...
if __name__ == "__main__":
    ...
    #my_yticks = ii
    gen_plot(my_yticks,my_yticks,one2one,one2all)
    ...
    for ele in my_yticks:
        node_yticks.append(list(node2index.keys())[list(node2index.values()).index(ele)])
    gen_plot(my_yticks,node_yticks,one2one,one2all)
